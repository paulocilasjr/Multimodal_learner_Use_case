...
CD3_ZIP_NAME  = "CD3_images.zip"
CD8_ZIP_NAME  = "CD8_images.zip"
...
def main():
    ...
    ap.add_argument("--cd3_zip",  default=None, help="Path to CD3_images.zip  (default: <data_root>/CD3_images.zip)")
    ap.add_argument("--cd8_zip",  default=None, help="Path to CD8_images.zip  (default: <data_root>/CD8_images.zip)")
    args = ap.parse_args()

    data_root = Path(args.data_root) if args.data_root else default_data_root()
    pdl1_zip = Path(args.pdl1_zip) if args.pdl1_zip else default_pdl1_zip()
    cd3_zip  = Path(args.cd3_zip)  if args.cd3_zip  else (data_root / CD3_ZIP_NAME)
    cd8_zip  = Path(args.cd8_zip)  if args.cd8_zip  else (data_root / CD8_ZIP_NAME)
    ...
    # --- Image paths (PDL1 required, CD3/CD8 optional)
    # PDL1 → 'image_path' (backward compatible)
    df_imgs_pdl1 = map_images_from_zip(pdl1_zip)
    df = df.merge(df_imgs_pdl1, on="patient_id", how="left")

    # CD3 → 'cd3_image_path'
    if cd3_zip.exists():
        df_imgs_cd3 = map_images_from_zip(cd3_zip).rename(columns={"image_path": "cd3_image_path"})
    else:
        df_imgs_cd3 = pd.DataFrame(columns=["patient_id", "cd3_image_path"])
    df = df.merge(df_imgs_cd3, on="patient_id", how="left")

    # CD8 → 'cd8_image_path'
    if cd8_zip.exists():
        df_imgs_cd8 = map_images_from_zip(cd8_zip).rename(columns={"image_path": "cd8_image_path"})
    else:
        df_imgs_cd8 = pd.DataFrame(columns=["patient_id", "cd8_image_path"])
    df = df.merge(df_imgs_cd8, on="patient_id", how="left")

    # --- Drop patients with no images at all (PDL1/CD3/CD8)
    before = len(df)
    img_cols = ["image_path", "cd3_image_path", "cd8_image_path"]
    df = df[df[img_cols].notna().any(axis=1)].copy()
    print(f"[OK] Dropped patients without any image: kept {len(df)}/{before}")
