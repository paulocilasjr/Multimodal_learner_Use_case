...
# --------------------------- Detailed metrics (embedded) ---------------------------
from sklearn.metrics import (
    accuracy_score, precision_score, recall_score, f1_score,
    average_precision_score, log_loss, confusion_matrix, matthews_corrcoef
)

def _compute_split_metrics(y_true, y_proba, y_pred):
    """Return dict of metrics for one split."""
    out = {}
    out["Accuracy"] = accuracy_score(y_true, y_pred)
    out["Precision"] = precision_score(y_true, y_pred, zero_division=0)
    out["Recall_(Sensitivity/TPR)"] = recall_score(y_true, y_pred, zero_division=0)
    out["F1-Score"] = f1_score(y_true, y_pred, zero_division=0)
    tn, fp, fn, tp = confusion_matrix(y_true, y_pred, labels=[0, 1]).ravel()
    out["Specificity_(TNR)"] = tn / (tn + fp) if (tn + fp) > 0 else 0.0
    out["ROC-AUC"] = roc_auc_score(y_true, y_proba) if y_proba is not None else np.nan
    out["PR-AUC"] = average_precision_score(y_true, y_proba) if y_proba is not None else np.nan
    try:
        if y_proba is not None:
            out["LogLoss"] = log_loss(y_true, np.vstack([1 - y_proba, y_proba]).T)
        else:
            out["LogLoss"] = np.nan
    except Exception:
        out["LogLoss"] = np.nan
    out["MCC"] = matthews_corrcoef(y_true, y_pred) if len(np.unique(y_true)) == 2 else np.nan
    return out


def evaluate_all_transparency_local(predictor: MultiModalPredictor,
                                    train_df: pd.DataFrame,
                                    val_df: pd.DataFrame,
    ...
    """
    Returns (metrics_table, details_dict)
    metrics_table: DataFrame indexed by metric with columns Train/Validation/Test.
    """
    def _eval(df):
        y_true = df[target_col].to_numpy().astype(int)
        y_proba = positive_class_proba(predictor.predict_proba(df))
        thr = 0.5 if threshold is None else float(threshold)
        if y_proba is not None:
            y_pred = (y_proba >= thr).astype(int)
        else:
            y_pred = predictor.predict(df).to_numpy().astype(int)
        return y_true, y_proba, y_pred

    y_tr, p_tr, yhat_tr = _eval(train_df)
    y_va, p_va, yhat_va = _eval(val_df)
    y_te, p_te, yhat_te = _eval(test_df)

    m_tr = _compute_split_metrics(y_tr, p_tr, yhat_tr)
    m_va = _compute_split_metrics(y_va, p_va, yhat_va)
    m_te = _compute_split_metrics(y_te, p_te, yhat_te)

    all_keys = [
        "Accuracy","Precision","Recall_(Sensitivity/TPR)","F1-Score",
        "Specificity_(TNR)","ROC-AUC","PR-AUC","LogLoss","MCC"
    ]
    table = pd.DataFrame(
        {"Train": [m_tr.get(k, np.nan) for k in all_keys],
         "Validation": [m_va.get(k, np.nan) for k in all_keys],
         "Test": [m_te.get(k, np.nan) for k in all_keys]},
        index=all_keys,
    )
    details = {"train": m_tr, "val": m_va, "test": m_te}
...
def main():
    ...
    for s in seeds:
        ...
        # Impute stats from TRAIN only (avoids leakage)
        ...
        # === Detailed metric table (Train / Validation / Test) ===
        try:
            metrics_table, _ = evaluate_all_transparency_local(
                predictor=predictor,
                train_df=df_train,
                val_df=df_val,
                test_df=df_test,
                target_col=label_col,
                threshold=None,  # decision at 0.5
            )
            print("\n=== Metrics (Train / Validation / Test) ===")
            print(metrics_table.round(6).to_string())
        except Exception as e:
            print(f"[warn] Could not compute detailed metrics: {e}")
